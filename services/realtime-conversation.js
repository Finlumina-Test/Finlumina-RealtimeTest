...
// Helper: PCM16 → μ-law 8-bit (Twilio requirement)
...
export function setupRealtime(app) {
  app.ws("/realtime", async (ws) => {
    ...
    const resp = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
      ...
      body: JSON.stringify({}), // no parameters
    });

    const keyData = await resp.json();
    console.log("🔑 OpenAI client secret response:", keyData);

    // ✅ Fix: accept either client_secret.value or value at top level
    const ephemeralKey = keyData?.client_secret?.value || keyData?.value;
    if (!ephemeralKey) {
    ...
    // 2️⃣ Connect to OpenAI Realtime with model & voice in query params
